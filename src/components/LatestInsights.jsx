    ...
    <section className="py-10 px-5 bg-gray-50 " data-aos="fade-up">
      <div className="max-w-[1200px] mx-auto">
          ...
          {insights.map((item, i) => (
            ...
            >
              ...
              <div className="p-6 flex h-[50%] flex-col flex-grow">
                <div className="mb-3">
                  <span className="bg-brand/10 text-brand text-[11px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">
                    {item.category}
                  </span>
                </div>
                <h3 className="text-xl font-bold mb-3 text-gray-800 group-hover:text-brand transition-colors">
                  {item.title}
                </h3>
                <div className="flex h-[0%] items-center justify-between text-gray-500 text-sm mt-auto pt-4 border-t border-gray-100">
                  <span>{item.date}</span>
                  <Link to={item.link}>
                    <span className="font-bold text-brand flex items-center gap-1 group-hover:gap-2 transition-all">
                      Read{" "}
                      <span className="hidden group-hover:inline">&rarr;</span>
                    </span>
                  </Link>
                </div>
              </div>
        ...
        <div className="mt-8 text-center md:hidden">
          <Link
            to="/about"
            className="inline-block text-brand font-bold hover:text-accent transition-colors"
          >

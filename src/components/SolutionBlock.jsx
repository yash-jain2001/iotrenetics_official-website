const SolutionBlock = ({ title, description, image, reverse = false, btnText = 'LEARN MORE' }) => {
  return (
    <div className={`flex items-center justify-between gap-15 ${reverse ? 'flex-row-reverse' : ''} max-md:flex-col`} data-aos="fade-up">
      <div className="bg-gray-100 p-10 rounded-xl max-w-lg shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg max-md:max-w-full">
        ...
        <p className="text-base leading-[1.8] mb-7 text-gray-800 text-justify">{description}</p>
      ...
      <div className={`${reverse ? 'w-4/5' : 'w-full'} max-w-[500px]`}>
        <img src={image} alt={typeof title === 'string' ? title : ''} loading="lazy" className="w-full" />
      </div>
    </div>
  );
};

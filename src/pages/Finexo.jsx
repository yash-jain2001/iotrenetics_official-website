...
/* ─── ICONS ──────────────────────────────────────────────────────────────── */
const I = {
  Mic: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="2" width="6" height="12" rx="3"/><path d="M5 10a7 7 0 0 0 14 0"/><line x1="12" y1="19" x2="12" y2="22"/><line x1="8" y1="22" x2="16" y2="22"/></svg>,
  Globe: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20"/></svg>,
  ...
  RefreshCw: () => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10"/><polyline points="1 20 1 14 7 14"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>,
  Swap: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>,
  Gold: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
...
/* ─── LIVE CONVERTER ─────────────────────────────────────────────────────── */
const CONV_RATES = {
  USD: 83.92, EUR: 91.45, GBP: 107.3, AED: 22.84,
  JPY: 0.562, SGD: 62.18, AUD: 54.73, CHF: 95.21,
  CAD: 61.47, KWD: 273.5, INR: 1,
};
    ...
    <div className="card" style={{ padding: "28px 28px", boxShadow: "0 8px 40px rgba(26,86,219,.07)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--sub)" }}>Real-time rates · in-app</p>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            onClick={swap}
            style={{ width: 36, height: 36, borderRadius: "50%", border: "1.5px solid var(--line-s)", background: "var(--white)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--accent)", transition: "background .15s" }}
          >
            <I.Swap />
          </button>
          <div style={{ flex: 1, height: 1, background: "var(--line-s)" }} />
...
/* ─── NET WORTH VIZ ──────────────────────────────────────────────────────── */
const ASSETS = [
  { label: "Bank Accounts", icon: <I.TrendUp />, color: "var(--accent)", pct: 56, amount: "₹1,12,000", tag: "digital" },
  { label: "Physical Gold", icon: <I.Gold />, color: "var(--gold)", pct: 44, amount: "₹88,000", tag: "210g" },
  { label: "Vehicle", icon: <I.Car />, color: "#7c5cbf", pct: 30, amount: "₹60,000", tag: "est. value" },
  { label: "Cash in Hand", icon: <I.Shield />, color: "#0a7c3e", pct: 18, amount: "₹36,000", tag: "multi-currency" },
  { label: "Property Share", icon: <I.Home />, color: "#c47f17", pct: 24, amount: "₹48,000", tag: "partial" },
];
  ...
  return (
    <div className="card" style={{ padding: "28px 28px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <span className="label label-gold"><I.Gold /> Physical Assets</span>
          <p className="big-num" style={{ marginTop: 10 }}>₹3,44,000</p>
          <p style={{ fontSize: 12, color: "var(--sub)", marginTop: 3 }}>Total net worth across all asset classes</p>
        </div>
      ...
      <div>
        {ASSETS.map((a, i) => (
          <div className="asset-row" key={i}>
            ...
            <span style={{ fontSize: 10, color: a.color, fontWeight: 700, background: a.color + "15", borderRadius: 99, padding: "2px 8px", marginLeft: 8, flexShrink: 0 }}>{a.tag}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
        <div style={{ display: "flex" }}>
          {SPLIT_PEOPLE.map((p, i) => (
            <div key={i} style={{ width: 28, height: 28, borderRadius: "50%", background: p.color, color: "#fff", fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", marginLeft: i > 0 ? -8 : 0, border: "2px solid var(--white)", zIndex: 4 - i, position: "relative" }}>{p.init}</div>
          ))}
        </div>
        <div>
      ...
      {SPLIT_PEOPLE.map((p, i) => (
        <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: i < 3 ? 10 : 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: p.color, color: "#fff", fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{p.init}</div>
      ...
      <div className="inner">
        <div style={{ display: "flex", alignItems: "center", gap: 72, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 420px", minWidth: 0 }}>
            ...
            <FadeUp className="d2">
              <p style={{ fontSize: 17, color: "var(--sub)", lineHeight: 1.75, maxWidth: 460, marginBottom: 36, fontWeight: 400 }}>
                Speak your expenses. Track physical assets. Split bills. Convert currencies live. 
                Finexo understands money the way you talk about it.
              </p>
            </FadeUp>
          ...
          <div style={{ flex: "1 1 340px", minWidth: 0, position: "relative", display: "flex", justifyContent: "center" }}>
            {/* Decorative rings */}
            {[460, 340].map((s, i) => (
              <div key={i} style={{ position: "absolute", width: s, height: s, borderRadius: "50%", border: `1px solid ${i === 0 ? "var(--accent-l)" : "var(--accent-m)"}`, opacity: .6, top: "50%", left: "50%", transform: "translate(-50%,-50%)", pointerEvents: "none" }} />
            ))}

            <img src="/assets/finexo1.webp" alt="Finexo App" loading="eager" className="float"
              style={{ width: "100%", maxWidth: 320, position: "relative", zIndex: 2, filter: "drop-shadow(0 32px 56px rgba(26,86,219,.14))" }}
            />
      ...
      <div className="inner">
          ...
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 20 }}>
            <div>
              ...
              <h2 style={{ fontFamily: "'DM Serif Display',serif", fontSize: "clamp(2rem,3.8vw,3rem)", color: "var(--ink)", margin: "14px 0 0", letterSpacing: "-.02em", lineHeight: 1.1 }}>
                One app.<br /><span style={{ fontStyle: "italic" }}>No spreadsheets.</span>
              </h2>
            </div>
            <p style={{ fontSize: 15, color: "var(--sub)", maxWidth: 340, lineHeight: 1.7, fontWeight: 400 }}>
              From daily chai to foreign trips — track, convert, split, and understand every rupee.
            </p>
          </div>
        </FadeUp>
        ...
        <div className="feat-grid fg-3" style={{ marginBottom: 14 }}>
          {[
            ...
            { icon: <I.Globe />, tag: "Real-time", title: "Live Currency Conversion", body: "Check live rates and save transactions in any currency. Auto-converts to your base currency using real-time data." },
            { icon: <I.Users />, tag: "Group finance", title: "Bill Splitting", body: "Split any bill among any group voice-first. Track who settled and who owes with one clear screen." },
          ].map((f, i) => (
        ...
        <div className="feat-grid fg-3" style={{ marginTop: 14 }}>
          {[
            { icon: <I.BarChart />, title: "Visual Reports", body: "Auto-generated weekly and monthly breakdowns with intuitive charts." },
            { icon: <I.Shield />, title: "Private by Design", body: "No ads, no data selling. Your financial data stays entirely yours." },
            { icon: <I.Brain />, title: "Ask Finexo AI", body: "\"What's my biggest expense this month?\" — get intelligent plain-language answers." },
          ].map((f, i) => (
            <FadeUp key={i} className={`d${i + 1}`}>
        ...
        <div style={{ display: "flex", gap: 72, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FadeUp style={{ flex: "1 1 360px", minWidth: 0 }}>
            ...
            <p style={{ fontSize: 16, color: "var(--sub)", lineHeight: 1.75, maxWidth: 420, marginBottom: 36 }}>
              No tapping into categories, no manual currency selection. Just say what happened and Finexo understands.
            </p>
          ...
          <FadeUp className="d2" style={{ flex: "1 1 320px", minWidth: 0 }}>
            ...
            {[
              ...
              { n: "03", title: "Insights arrive", body: "Weekly summaries and AI answers — always ready.", accent: "var(--green)" },
            ].map((s, i) => (
              <div key={i} style={{ display: "flex", gap: 16, alignItems: "flex-start", marginBottom: i < 2 ? 20 : 0 }}>
                <div style={{ position: "relative", flexShrink: 0 }}>
                  <div style={{ width: 42, height: 42, borderRadius: "50%", background: s.accent + "15", color: s.accent, border: `1px solid ${s.accent}30`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Serif Display',serif", fontSize: 15, fontStyle: "italic" }}>{s.n}</div>
    ...
    <section className="sec" style={{ background: "var(--bg)" }}>
      <div className="inner">
        <div style={{ display: "flex", gap: 72, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <FadeUp style={{ flex: "1 1 320px", minWidth: 0, display: "flex", justifyContent: "center" }}>
            <img src="/assets/finexo2.webp" alt="Net Worth" loading="lazy" className="float"
              style={{ width: "100%", maxWidth: 380, filter: "drop-shadow(0 24px 48px rgba(0,0,0,.09))" }} />
          </FadeUp>

          <FadeUp className="d1" style={{ flex: "1 1 380px", minWidth: 0 }}>
            <span className="label label-gold"><I.Gold /> Physical Assets</span>
            <h2 style={{ fontFamily: "'DM Serif Display',serif", fontSize: "clamp(2rem,3.5vw,2.8rem)", color: "var(--ink)", margin: "16px 0 14px", lineHeight: 1.1, letterSpacing: "-.02em" }}>
              Your wealth isn't<br />
              <span style={{ fontStyle: "italic", color: "var(--gold)" }}>only in your bank.</span>
            </h2>
            <p style={{ fontSize: 16, color: "var(--sub)", lineHeight: 1.75, maxWidth: 440, marginBottom: 28 }}>
              Most finance apps stop at your bank balance. Finexo goes further — add gold, vehicles, property shares, and cash holdings for a true picture of what you own.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                { icon: <I.Gold />, label: "Physical Gold", sub: "Grams, coins, jewellery", c: "var(--gold)" },
                { icon: <I.Car />, label: "Vehicles", sub: "Estimated market value", c: "#7c5cbf" },
                { icon: <I.Home />, label: "Property", sub: "Partial or full share", c: "var(--accent)" },
                { icon: <I.Shield />, label: "Cash Holdings", sub: "Any currency", c: "var(--green)" },
              ].map((a, i) => (
                <div key={i} className="card" style={{ padding: "14px 16px", display: "flex", gap: 10, alignItems: "flex-start" }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: a.c, flexShrink: 0, marginTop: 6 }} />
                  <div>
                    <p style={{ fontWeight: 600, fontSize: 13, color: "var(--ink)", marginBottom: 2 }}>{a.label}</p>
      ...
      <div className="inner">
        <div style={{ display: "flex", gap: 72, alignItems: "center", flexWrap: "wrap" }}>
          <FadeUp style={{ flex: "1 1 400px", minWidth: 0 }}>
            ...
            <p style={{ fontSize: 16, color: "var(--sub)", lineHeight: 1.75, maxWidth: 440, marginBottom: 28 }}>
              Finexo knows your entire financial history — every voice entry, every split, every currency transaction — and answers in plain language.
            </p>
          ...
          <FadeUp className="d2" style={{ flex: "1 1 300px", minWidth: 0, display: "flex", justifyContent: "center" }}>
            <div style={{ position: "relative", width: "100%", maxWidth: 340 }}>
              <img src="/assets/finexo3.webp" alt="Finexo AI" loading="lazy" className="float"
                style={{ width: "100%", filter: "drop-shadow(0 24px 48px rgba(26,86,219,.11))" }} />
              {/* Chat overlay */}
              ...
              <div className="bub bub-ai" style={{ position: "absolute", right: -12, bottom: "24%" }}>
                <span style={{ fontSize: 10, color: "rgba(255,255,255,.5)", fontWeight: 600, display: "block", marginBottom: 3 }}>Finexo AI</span>
      ...
      <div className="inner" style={{ textAlign: "center", position: "relative", zIndex: 1 }}>
        <FadeUp>
          ...
          <h2 style={{ fontFamily: "'DM Serif Display',serif", fontWeight: 400, fontSize: "clamp(2.4rem,5vw,3.8rem)", color: "#fff", margin: "22px 0 18px", lineHeight: 1.08, letterSpacing: "-.025em" }}>
            Start knowing<br />
            <span style={{ fontStyle: "italic", color: "#93b4fd" }}>exactly where you stand.</span>
          </h2>
          <p style={{ fontSize: 17, color: "rgba(255,255,255,.55)", maxWidth: 420, margin: "0 auto 40px", lineHeight: 1.75, fontWeight: 400 }}>
            Voice tracking, physical asset net worth, group splits, live currency conversion — all in one beautifully simple app.
          </p>
          <a href="https://play.google.com/store/apps/details?id=com.iotrenetics.finexo" target="_blank" rel="noopener noreferrer">
